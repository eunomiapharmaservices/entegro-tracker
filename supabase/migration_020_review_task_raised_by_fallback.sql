-- Migration: the auto-generated review task now falls back to whoever is
-- named in "Raised by" (matched to a People entry by name) when no
-- Reviewer is explicitly set, instead of staying unassigned.
