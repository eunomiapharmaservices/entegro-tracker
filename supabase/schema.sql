...
begin
  if new.is_review_task and new.review_of_task_id is not null
    ...
    -- Extend the main task's due date, and move it back to In progress
    -- (out of In review) now that its review is done.
    update tasks
    ...
    -- Copy every comment from the review task onto the main task, so the
    -- review discussion becomes part of its permanent history too.
